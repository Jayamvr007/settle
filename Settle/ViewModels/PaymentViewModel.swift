...
class PaymentViewModel: ObservableObject {
    ...
    func recordCashPayment(settlement: SimplifiedSettlement, group: Group, notes: String? = nil, transactionId: String? = nil) {
        // Create a "settlement expense" to offset the balance
        // When "from" pays "to", we record an expense where:
        // - "from" paid the amount
        // - Only "to" is in the split (receives the payment)
        // This effectively cancels out the debt
        
        let settlementExpense = Expense(
            id: UUID(),
            title: "💰 Settlement: \(settlement.from.name) → \(settlement.to.name)",
            amount: settlement.amount,
            date: Date(),
            category: .general,
            notes: notes ?? "Settlement payment\(transactionId != nil ? " (TxnID: \(transactionId!))" : "")",
            paidBy: settlement.from,  // The person who owed (and is now paying)
            shares: [
                ExpenseShare(
                    id: UUID(),
                    member: settlement.to,  // The person who was owed gets the "share"
                    amount: settlement.amount
                )
            ]
        )
        ...
        GroupRepository.shared.addExpense(settlementExpense, to: group)
    }
